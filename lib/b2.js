...
function B2(options) {
    ...
    /*
        allows an optional axiosConfig object that overrides the default axios config
        creates new axios instance so that axios-retry isn't injected into the user's code if they also use axios
    */
    const axiosClient = axios.create(Object.assign({ /* no defaults for now */ }, options.axiosConfig));
    /*
        allows an optional retryConfig object that overrides the default retry behaviour
    ...
    */
    axiosRetry(axiosClient, Object.assign({ retries: 3 }, options.retryConfig));
